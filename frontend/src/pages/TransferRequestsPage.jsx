...
export default function TransferRequestsPage() {
  ...
  const load = useCallback(() => {
    ...
    api.get("/transfer-requests", { params })
  ...
  const approve = async (req) => {
    try {
      await api.put(`/transfer-requests/${req.id}/approve`);
  ...
  const confirmReject = async () => {
    try {
      await api.put(`/transfer-requests/${rejectTarget.id}/reject`, null, {
    ...
    <div className="p-6 lg:p-10 max-w-[1000px] mx-auto">
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="rounded-none">
          <DialogHeader>
            <DialogTitle>{t("tr_reject_title")}</DialogTitle>
          </DialogHeader>
